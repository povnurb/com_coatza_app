<template>
  <div>
        ...
        <div class="row pull-right">
          <div class="col-12">
            <base-button
              ...
              @click="addNewWidget()"
            >
              Agregar Widget  <!--nombre del boton-->
            </base-button>
          </div>
        </div>
      </card>
    ...
    <!-- SAVE TEMPLATE aparecera el formulario solo si existe algo que salvar-->
    <div class="row" >
      <card>
        <div slot="header">
          <h4 class="card-title">Salvar Tema</h4>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      iotIndicatorConfig: {
        ...
        variableSendFreq: "30",
        class: "success",
        widget: "indicator",          //https://glyphsearch.com/?query=termo
        icon: "fa-thermometer-half", //fa-bath, fa-thermometer-full, fa-sun, fa-thermometer,fa-th,fa-lock,fa-thermometer-half, fa-water,fa-walking,fa-door-open,fa-bolt,fa-exclamation-triangle,fa-digital-tachograph,fa-hdd,fa-hot-tub,fa-tint,fa-wrench, fa-car-battery
        column: "col-6"
      },
  ...
  methods: {
    ...
    async deleteTemplate(template){
      if(confirm("¿Deseas borrar el tema " + template.name + "? Esta acción no se puede deshacer")){
      const axiosHeaders = {
        headers: {
          token: this.$store.state.auth.token
        },
      ...
      try {
        ...
        if (res.data.status == "fail" && res.data.error == "tema en uso") {
          this.$$notify({
            type: "danger",
            icon: "tim-icons icon-alert-circle-exc",
            message: template.name + " !El tema esta en uso. primero remueve el dispositivo asociado al tema!"
          });
        ...
        if (res.data.status == "success") {
          this.$notify({
            type: "success",
            icon: "tim-icons icon-alert-circle-exc",
            message: template.name + " ha sido borrado!"
          });
        ...
        this.$notify({
          type: "danger",
          icon: "tim-icons icon-alert-circle-exc",
          message: "Error al adquirir los temas ..."
        });
        console.log(error);
        return;
      }
