    ...
    >
      <template slot-scope="" slot="links">
        <sidebar-item
          :link="{
            name: 'Panel',
            icon: 'tim-icons icon-laptop', //icon-chart-pie-36
            path: '/panel'
          }"
        >
        </sidebar-item>
        ...
        <sidebar-item
          :link="{
            name: 'Reglas de Alarmas',  //Alarmas
...
export default {
  ...
  data() {
    return {
      ...
      options: {
        ...
        clientId:"web_" + this.$store.state.auth.userData.email + "_" + Date.now(),
        username: "",
        password: ""
      }
    };
  ...
  computed: {
    ...
    this.initScrollbar();
    setTimeout(() => {this.startMqttClient();}, 2000); //arranca el mqtt
  },
  beforeDestroy() {
    this.$nuxt.$off("mqtt-sender"); //se agrego por que faltaba si hay error se quita
  },
  methods: {
    ...
    async getMqttCredentials() {
      ...
      this.options.password = credentials.data.password;
        ...
        console.log(error);
        if(error.respose.status == 401){ //o toSend
